...
\li \For $stock \gets 1$ to $count$ 
	\Do
\li 		$limit[stock] = min\{limit[stock],\ total / start[stock]\}$
    	\End
\li profit is hashtable
\li \For $cash \gets 0$ to $total$ 
	\Do
\li     		$profit[cash,\ 0,\ 0] \gets cash$
\li    		 $stock \gets 1$ to $count$
\li    		\For $l \gets 0$ to $limit[stock]$ \Do
\li	    		$profit[cash,\ stock,\ l] \gets profit[cash,\ stock - 1,\ limit[stock - 1]]$
\li	    		\If $start[stock] <= cash$ and $l > 0$ 
				\Then
\li	       			$current \gets end[stock] + profit[cash - (l * start[stock]),\ stock,\  l - 1]$
\li	      	 		$profit[cash,\ stock,\ l] \gets max\{profit[cash,\ stock,\ l],\ current\}$
   				 \End 
		\End 
	\End
\li \Return \id{profit}[cash,\ count,\ limit[count]]
\end{codebox}

If we consider a stock's minimum price as \proc{1}, runtime is \proc{O}($total^2 * count$).

Recurrence relation below.

\begin{codebox}
\Procname{$\id{profit}[c, s, l]$}
\li    \If $s \gets 0$
	\Then 
\li 		$c$
	\End
\li    \Else \If $l \gets 0$ or $c < start[s]$
 	\Then 
\li		$profit[c, s - 1, limit[s - 1]]$
	\End
\li    \Else 
	\Then
\li 		$max\{profit[c, s - 1, limit[s - 1]], end[s] + profit[c - start[s], s, l - 1]\}$
	\End
\end{codebox}

Recurrence relation represents problem from top to bottom. Considers all profit 
possibilities at given amount and finds best stock to buy. Then buys the best
stock as much as it can at given amount. Repeats this cycle till it can't buy anything.
