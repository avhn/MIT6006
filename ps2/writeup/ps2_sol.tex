...
For 3D rendering without hardware acceleration, algorithm complexity
is proportional to surface area. Regarding this, a node's area at level i
of recursion tree, is proportional to $\frac{1}{3}^{i+1}$
which is length of one edge of the triangle. For a triagles' area,
we can say area is just about to edge squared as $\frac{1}{9}^{i+1}$.
So cost of a node at level i is $\mathcal{O}(\frac{1}{9}^{i+1}) = \mathcal{O}(1)$.
At level i, total node amount to render is $3 * 4^i$.
The cost of total rendering regarding every level of recursion tree is:
$\ \displaystyle\sum_{i = 0}^{n}{ (3 * 4^i) * 1} = \mathcal{O}(1)$
