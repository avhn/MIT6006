...
KTH\_ROOT(A, K, N):
    ...
    \section{do}
    \section{\quad i = i + 1}
    \section{\quad B_i = ADD(B_{i-1},\ B_{i-1},\ N)}
    \section{till B[N+1] $>$ 0 or POW(B_i,\ K) >= A}
    \section{R = ZERO(N)}
    \section{for j = i - 1 to 0}
    \section{\quad R' = ADD(R,\ B_i)}
    \section{\quad if POW(R',\ K) $<$ A:}
    \section{\qquad R = R'}
    \section{return R}
POWER(B, E):
    \section{P = COPY(B)}
